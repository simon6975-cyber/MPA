# MPA (Mobile Photo Album) v1.10

모바일 포토앨범 제작 서비스 · Firebase 연동 + 상품 관리 화면

## 🆕 v1.10 변경사항

- **관리자 상품 관리 페이지 추가** (`/admin/products`)
  - 3개 티어(미니/스탠다드/프리미엄)의 이름·장수·가격·활성화 편집
  - 6개 기본 커버 색상 + 추가/삭제/순서 변경, HEX 컬러 피커
  - 배송비·무료배송 기준·선물 포장 추가 금액 설정
  - 저장 시 Firestore `settings/product` 문서에 기록, 고객 앱에 실시간 반영
- 고객 앱의 하드코딩된 `TIERS`/`COLORS`를 Firestore 구독으로 전환 (폴백값 유지)
